...
//#pragma omp parallel for collapse (2)
		for (int j=0; j<static_cast<int>(l.ny); ++j)
		{
			for (int i=0; i<static_cast<int>(l.nx); ++i)
			{
				...
				// std::vector<double> f_eq_bosch(9,0);
				// f_eq_bosch[0] = (-2*rho*(-2 + 3*u*u + 3*v*v))/9.;
				// f_eq_bosch[1] = (rho*(2 + 6*u + 6*u*u - 3*v*v))/18.;
				// f_eq_bosch[2] = (rho*(2 - 3*u*u + 6*v + 6*v*v))/18. ;
				// f_eq_bosch[3] = (rho*(2 - 6*u + 6*u*u - 3*v*v))/18.;
				// f_eq_bosch[4] = (rho*(2 - 3*u*u - 6*v + 6*v*v))/18.;
				// f_eq_bosch[5] = (rho*(1 + 3*u*u + 3*v + 3*v*v + u*(3 + 9*v)))/36.;
				// f_eq_bosch[6] = (rho*(1 + 3*u*u + 3*v + 3*v*v - 3*u*(1 + 3*v)))/36.;
				// f_eq_bosch[7] = (rho*(1 + 3*u*u - 3*v + 3*v*v + u*(-3 + 9*v)))/36.;
				// f_eq_bosch[8] = (rho*(1 + 3*u*u + u*(3 - 9*v) - 3*v + 3*v*v))/36.;
				// std::cout <<std::setprecision(20);
				// std::cout << std::fixed;
				// std::cout << "Comparison of F_EQ" << std::endl;
				// std::cout << "Bösch (f_eq) \t Our code (f_eq) \t difference" << std::endl;
				// for(unsigned k=0; k<9; k++){
				// 	std::cout << f_eq_bosch[k] << "\t\t" << l.get_node(i,j).f(k) << "\t\t" << (l.get_node(i,j).f(k)-f_eq_bosch[k]) << std::endl;
				// }
				// std::cout << std::endl;

				// std::vector<double> f_bosch(9,0);
				// f_bosch[0] = n.f(0);
				// f_bosch[1] = n.f(1);
				// f_bosch[2] = n.f(2);
				// f_bosch[3] = n.f(3);
				...
				// std::cout <<std::setprecision(20);
				// std::cout << std::fixed;
				// std::cout << "Comparison of f_i" << std::endl;
				// std::cout << "Bösch (f_i) \t Our code (f_i) \t difference" << std::endl;
				// for(unsigned k=0; k<9; k++){
				// 	std::cout << f_bosch[k] << "\t\t" << f_old[k] << "\t\t" << (f_old[k]-f_bosch[k]) << std::endl;
				// }
				// std::cout << std::endl;
