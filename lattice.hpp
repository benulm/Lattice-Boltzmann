...
enum wall_orientation {left, right, top, bot};
...
public: // members
	...
	std::vector<wall_orientation> obstacle; ///< Vector to store where in the wall we are
...
void lattice::write_vtk(std::string file_name)
{
	std::ofstream ofs(file_name.c_str());
	if (ofs.is_open())
	{
		...
		for (unsigned int j=0; j<ny; ++j)
		{
			for (unsigned int i=0; i<nx; ++i) 
			{
				ofs << std::scientific << nodes[(j+buffer_size)*real_nx + i + buffer_size].u() << "\n";
			}
		} 
	}
	else throw std::runtime_error("could not write to file");
